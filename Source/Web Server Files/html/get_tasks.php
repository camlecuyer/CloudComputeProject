...
<?php
	...
	else
	{
		...
		$result = mysqli_query($connection, "SELECT TaskID, DivisionID, BuildingID, IssueID, TaskDateOpened, TaskDesc, TaskFloorNumber, TaskLocation FROM Tasks WHERE TaskIsClosed = 0 ORDER BY TaskDateOpened ASC LIMIT 15");
		...
		if(!empty($result))
		{
			if(mysqli_num_rows($result) > 0)
			{
				...
				while($row = mysqli_fetch_array($result))
				{
					...
					$issues["TaskDateOpened"] = $row["TaskDateOpened"];
